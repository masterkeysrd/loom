...
// Checkpointer persists graph checkpoints in a SQLite database.
...
func (c *Checkpointer) Load(ctx context.Context, location graph.Location) (*graph.Checkpoint, error) {
	...
	var stateStr, nextStr, timestampStr string
	...
	record.State = json.RawMessage(stateStr)
	record.Next = json.RawMessage(nextStr)

	record.Timestamp, err = time.Parse(time.RFC3339Nano, timestampStr)
	if err != nil {
		return nil, fmt.Errorf("sqlite checkpointer: parse timestamp %q: %w", timestampStr, err)
	}
