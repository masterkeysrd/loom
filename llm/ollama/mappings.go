...
func toAPIMessage(msg message.Message) (api.Message, error) {
	...
	case *message.Tool:
		...
		isError = v.IsError
		if len(v.Content) == 0 && v.StructuredContent != nil {
			data, _ := json.Marshal(v.StructuredContent)
...
func toAssistantChunk(resp api.ChatResponse) (message.AssistantChunk, error) {
	...
	if resp.PromptEvalCount >= 0 || resp.EvalCount >= 0 {
		chunk.Metrics = &message.TokenMetrics{
			...
			Timing: message.TimingMetrics{
				Total:      resp.TotalDuration,
				Processing: resp.PromptEvalDuration,
				Generation: resp.EvalDuration,
			},
		}
	}
